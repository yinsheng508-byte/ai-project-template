...
6. 当前轮次统一在 `docs/current/` 中推进
7. 所有重要决策、review、escalation 必须记录到 `docs/current/`
8. 先看索引文件，再按当前阶段按需读取文档
9. 修改后必须执行最相关验证
10. Gemini 在 review 阶段只负责审查，不直接修改代码
11. 每个任务必须有可演示路径；HITL 任务在执行前需人工确认再交 agent
...
1. 再读 `README.md`
2. 再读 `docs/INDEX.md`
3. 按当前阶段读取 `docs/current/` 下最相关的文件
4. 不要默认一次性读完整个仓库
...
## 归档规则
- 当前轮次完成后，将 `docs/current/` 归档到 `docs/archive/<日期-名称>/`
